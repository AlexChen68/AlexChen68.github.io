...
因为虚拟机增加了一层虚拟硬件层，运行在虚拟机上的应用程序在进行数值计算时是运行在 Hypervisor 虚拟的 CPU 上的；另外一方面是由于计算程序本身的特性导致的差异。虚拟机虚拟的 cpu 架构不同于实际 cpu 架构，数值计算程序一般针对特定的 cpu 架构有一定的优化措施，虚拟化使这些措施作废，甚至起到反效果。比如对于本次实验的平台，实际的 CPU 架构是 2 块物理 CPU。

## 参考资料

[Java 全栈知识体系-Docker 部分](https://pdai.tech/md/devops/docker/docker-01-docker-vm.html)

[遇见狂神说](https://www.bilibili.com/video/BV1og4y1q7M4)

[霸气小闫-Docker 快速入门总结笔记](https://blog.csdn.net/Mr_YanMingXin/article/details/119504925)
